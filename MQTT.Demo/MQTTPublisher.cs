    ...
    public async Task ConnectAndPublishAsync(string device)
    {
        try
        {
            ...
            var options = new MqttClientOptionsBuilder()
                   ...
                   .WithWebSocketServer(c =>
                   {
                       c.WithUri("ws://localhost:5223/mqtt"); // ADDED THE MISSING SLASSES (//)
                       c.WithRequestHeaders(new Dictionary<string, string>
                       {
                           { "TenantId", device },
                           { "Authorization", $"Bearer {device}" }
                       });
                   })
                .WithCredentials(device, device) // Using device ID as both username and password
                .WithClientId(device)
        ...
        {
            Console.WriteLine($"Error publishing: {ex.Message} {ex.StackTrace}");
        }
